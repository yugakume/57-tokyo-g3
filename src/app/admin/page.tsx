...
type Tab = "links" | "categories" | "accounts" | "users" | "roles" | "announcements" | "announcementCategories" | "countdowns" | "branch" | "wallet";
...
function WalletManagementTab({
  ...
  staffProfiles: import("@/types").StaffProfile[];
  onToast: (msg: string) => void;
}) {
  const sorted = [...staffProfiles].sort((a, b) =>
    (a.fullName || a.lastName).localeCompare(b.fullName || b.lastName)
  );
  const registered = sorted.filter(p => p.walletAddress);
  const notRegistered = sorted.filter(p => !p.walletAddress);

  const copyAll = () => {
    const text = registered
      .map(p => `${p.fullName || p.lastName}\t${p.walletAddress}`)
      .join("\n");
  ...
  return (
    <div className="p-4 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-semibold text-gray-900 dark:text-gray-100">MetaMaskウォレット管理</h2>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            登録済み {registered.length} 名 / 未登録 {notRegistered.length} 名
          </p>
        </div>
        {registered.length > 0 && (
          <button
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-4 py-2.5 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
          <span className="text-xs font-semibold text-gray-600 dark:text-gray-400">✓ 登録済み（{registered.length}名）</span>
        </div>
        {registered.length === 0 ? (
          <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-6">まだ誰も登録していません</p>
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {registered.map(p => (
              <div key={p.id} className="flex items-center gap-3 px-4 py-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{p.fullName || p.lastName}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{p.email}</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20 px-2 py-1 rounded">
                    {p.walletAddress!.slice(0, 6)}...{p.walletAddress!.slice(-4)}
                  </span>
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(p.walletAddress!);
                      onToast(`${p.fullName || p.lastName}のアドレスをコピーしました`);
                    }}
                  ...
                  <a
                    href={`https://etherscan.io/address/${p.walletAddress}`}
                    target="_blank"
      ...
      {notRegistered.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="px-4 py-2.5 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
            <span className="text-xs font-semibold text-gray-500 dark:text-gray-500">未登録（{notRegistered.length}名）</span>
          </div>
          <div className="divide-y divide-gray-100 dark:divide-gray-700">
            {notRegistered.map(p => (
              <div key={p.id} className="flex items-center gap-3 px-4 py-3 opacity-60">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-gray-700 dark:text-gray-300">{p.fullName || p.lastName}</p>
                  <p className="text-xs text-gray-400">{p.email}</p>
                </div>
                <span className="text-xs text-gray-400 dark:text-gray-500">未登録</span>
              </div>
            ))}
          </div>
        </div>
